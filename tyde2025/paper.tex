...
it is not clear whether such a type is justified. Unlike mixed
inductive/coinductive types~\cite{hancock-stream}, we cannot directly write it
as the terminal coalgebra of some functor. If we were to parametrise
\AgdaDatatype{Expr} with \AgdaRecord{NExpr} and \AgdaField{pred}, then we would
have $\AgdaField{pred} : \AgdaDatatype{Expr}\ \AgdaRecord{NExpr}\

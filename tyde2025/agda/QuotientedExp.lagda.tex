...
module E where
  ...
  infixr 8 _^_
\end{code}
For exponentiation, we add constructors
\begin{code}
  data Expr where
\end{code}
\begin{code}[hide]
...
\begin{code}
    ...
    block-annihₗ : ∀ x → block zero x ≡ zero
\end{code}
Relate exponentiation and \AgdaInductiveConstructor{block}
\begin{code}
    ^-sucₗ : ∀ x y → suc x ^ y ≡ suc (block x y)
\end{code}
...
\begin{code}[hide]
  ...
  sucᴾ = oneᴾ +ᴾ_
\end{code}
Here we define the predecessor of exponentiation to use \AgdaInductiveConstructor{block}
\begin{code}
  ^-pred :
...
\end{code}
We can define the predecessor of \AgdaInductiveConstructor{block} using addition
and multiplication
...
After defining pred for the properties, it is straightforward to change the
proof to derive exponentiation and its properties for \AgdaRecord{ℕ∞}
\begin{code}[hide]
  block-isPred :
    ∀ {x x' y y'} → IsPred x' x → IsPred y' y →

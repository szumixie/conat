...
\end{code}
The operations and equations that we have in \AgdaDatatype{Expr} is enough to
...
that \AgdaRecord{ℕ∞} forms an exponential commutative semiring.

After defining the predecessors for the properties too, it is straightforward to
change the proof to derive exponentiation and its properties for
\AgdaRecord{ℕ∞}.

Every other property from the commutative semiring section stays the same, the
same proofs still work. It is also proven that extending the language with
exponentiation still retains its stability and completeness. We had to also extend to motive
and the structure to include exponentiation. Instantiating the structure with our new extended
\AgdaDatatype{Expr} we get all the proofs contained in it for conatural numbers and concluding
our proof that conatural numbers with $\AgdaFunction{\_+\_}$,$\AgdaFunction{\_\times\_}$ and
$\AgdaFunction{\_\textasciicircum\_}$ form an exponential semiring.

Here we can see an example proof of $2^\infty = \infty$ using conatural numbers

...
specify what the predecessor is for a particular element. As examples, \AgdaFunction{zero} does
...
If we naïvely try define multiplication by using addition, then Agda will reject
it, unless we use the unsafe \AgdaKeyword{TERMINATING} pragma:
\begin{code}[hide]
infixl 7 _×_
mutual
\end{code}
...
is productive but not guarded, as the recursive call to \AgdaFunction{\_×\_} is
under \AgdaFunction{\_+\_}, which is not a constructor.

We can prove some of the usual properties of addition over the conatural
...
the \AgdaKeyword{TERMINATING} pragma. Let us assume we have this equation
...
exponential semiring by guarded corecursion. Along the way, we show three
different methods starting out with the formalisation, showcasing some of the
difficulties and problems. The last method we show is the one we use to prove
that the conatural numbers form an exponential semiring.
...
we can define the injectivity proof for the \AgdaField{pred} destructor the
following way:

...
The intuition for the commutativity proof is that we keep track of how many
steps we have taken so far using a finite natural number. As we can see in the
diagram below, where the top line is the left hand side, the botton line is the
right hand side, at the beginning we go through $x$ and $y$ at the same time,
while learning that both of them must be at least $n$. At each step, we decrease
$x$ and $y$ and increase $n$ by one:
\begin{center}
...
In the second lemma, we similarly have to generalize the equation to avoid the

...
we need to also say that the equations of \AgdaDatatype{Expr} are proof
...
We define an inductive predicate $\AgdaDatatype{IsPred}\ x'\ x$ for convenience,
...
\AgdaDatatype{Expr}. The one below is the method for addition:
\begin{code}[hide]
  mutual
\end{code}
\begin{code}
...
With this, we can extract the familiar \AgdaFunction{predᴱ} in
...
the \AgdaInductiveConstructor{embed} constructor.
\begin{code}[hide]
mutual
\end{code}
\begin{code}
